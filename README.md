...
Start the Development ServerUsing npm:
npm start
...
6. Drag-and-Drop for Task Reordering

Decision: Implemented drag-and-drop functionality in TaskManagementDashboard and TaskCard using HTML5 Drag and Drop API.
Reason: Enhances user experience by allowing intuitive task reordering. State management in TaskManagementDashboard updates the task array on drop events.
Impact: Adds interactivity but increases complexity due to event handling and UI feedback (e.g., isDragging state and visual cues).
...
2. HTML5 Drag and Drop vs. Library

Trade-off: Used native HTML5 Drag and Drop API instead of a library like react-dnd.
Pros: Reduces dependencies, lightweight implementation, sufficient for simple task reordering.
Cons: Limited features (e.g., no complex drag interactions), requires manual event handling, and may have browser compatibility quirks.
